...
<%  String input=request.getParameter("p");
	...
	try{
		...
		Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/onlinefoodorderdb?autoReconnect=true&useSSL=false","root","0804"); 
				...
				if((selectedID>=1)&&(selectedID<=9))
   				{	
   					...
   					rs.next();
   						...
   						ps=con.prepareStatement("insert into foodOrder (order_id, food_id, quantity, total_price) values (?, ?, ?, ?)");
   			 	...
   			 	else if((selectedID>=101)&&(selectedID<=103)) 
   				{
					...
					rs.next();
   					...
   					int drink_price=rs.getInt(3);
   						...
   						ps=con.prepareStatement("insert into drinkOrder (order_id, drink_id, quantity, total_price) values (?, ?, ?, ?)");
